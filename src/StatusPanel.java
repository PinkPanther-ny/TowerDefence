...
public class StatusPanel {
    ...
    private final static Image fastforwardSign = new Image("res\\images\\fastforward.png");
    private final static Image fastbackwardSign = new Image("res\\images\\fastbackward.png");
    private final static Point FFS_Location = new Point(150, 720);
    private final static Point FBS_Location = new Point(110, 720);
    ...
    public void drawPanel(Input input, int timeScale, double delayRemain, Game game){
        //fastbackwardSign.drawFromTopLeft(FFS_Location.x, FFS_Location.y);

        if (pauseTimeScale==-1) {

            pauseSign.drawFromTopLeft(PS_Location.x, PS_Location.y);

            if (input.wasPressed(Keys.K) && game.getTimeScale() - 1 >= 0) {
